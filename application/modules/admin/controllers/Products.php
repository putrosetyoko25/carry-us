...
class Products extends CI_Controller
{
    ...
    public function add_product()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $sku = create_product_sku($name, $category_name, $price);
            ...
            $product['is_available'] = $this->input->post('is_available');
            $product['add_date'] = $date;

            $this->product->add_new_product($product);
            // CUYYYYYY
            $produkk = $this->db->where('sku', $sku)->get('products')->result_array();
            $courses['id_kursus'] = $produkk[0]['id'];
            $courses['nama_courses'] = $name;
            $courses['is_available'] = $this->input->post('is_available');
    ...
    public function edit($id = 0)
    {
        if ($this->product->is_product_exist($id)) {
            ...
            $produkk = $this->db->where('id', $id)->get('products')->result_array();
            $params['title'] = 'Edit ' . $data->name;

            $product['flash'] = $this->session->flashdata('edit_product_flash');
            $product['product'] = $data;
            $product['courses'] = $this->db->where('id_kursus', $id)->get('courses')->result_array();
            $product['categories'] = $this->product->get_all_categories();
    ...
    public function edit_product()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $produkk = $this->db->where('id', $id)->get('products')->result_array();
            $courses['nama_courses'] = $name;
            $courses['is_available'] = $available;
            $this->db->where('id_kursus', $id)->update('courses', $courses);
            // $this->db->insert('courses', $courses);
    ...
    public function manage($id = 0)
    {
        if ($this->product->is_product_exist($id)) {
            $data = $this->product->product_data($id);
            $product['list'] = $this->db->where('id_kursus', $id)->get('kursus')->result_array();
            $product['id'] = $id;
            $params['title'] = 'Manage ' . $data->name;
            ...
            $this->load->view('header', $params);
            $this->load->view('products/manage', $product);
            $this->load->view('footer');
        } else {
            show_404();
        }
    ...
    public function addcourse($id_kursus)
    {
        $params['title'] = 'Add Course';
        $product['flash'] = $this->session->flashdata('edit_product_flash');
        $product['id_kursus'] = $id_kursus;
        $product['produk'] = $this->db->where('id', $id_kursus)->get('products')->result_array();
        $this->load->view('header', $params);
        $this->load->view('products/addcourse', $product);
        $this->load->view('footer');
    }
    ...
    public function add_course()
    {
        ...
        $this->form_validation->set_rules('judul', 'Judul Course', 'trim|required');
        $this->form_validation->set_rules('link', 'Link Course', 'trim|required');

        if ($this->form_validation->run() == FALSE) {
            $id_kursus = $this->input->post('id_kursus');
            $this->addcourse($id_kursus);
        } else {
            $id = $this->input->post('id_kursus');
            $kursus['id_kursus'] = $id;
            $kursus['no_urut'] = $this->input->post('no_urut');
            $kursus['judul'] = $this->input->post('judul');
            $kursus['link'] = $this->input->post('link');

            $this->db->insert('kursus', $kursus);
            $this->session->set_flashdata('edit_product_flash', 'Produk berhasil diperbarui!');

            redirect('admin/products/manage/' . $id);
        }
    }
    ...
    {
        if ($this->product->is_course_exist($id)) {
            $product['buff'] = $this->db->where('id', $id)->get('kursus')->result_array();
            $product['kursus'] = $product['buff'][0];
            // var_dump($product); die;
            // $params['title'] = 'Edit ' . $data->name;
            $params['title'] = 'Edit Course';
            $product['flash'] = $this->session->flashdata('edit_product_flash');
            // $product['product'] = $data;
            $product['categories'] = $this->product->get_all_categories();

            $this->load->view('header', $params);
            $this->load->view('products/editcourse', $product);
            $this->load->view('footer');
        } else {
            show_404();
        }
    ...
    public function edit_course()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->db->where('id', $id)->update('kursus', $kursus);
            // $this->db->insert('courses', $courses);
            $this->session->set_flashdata('edit_product_flash', 'Produk berhasil diperbarui!');

            redirect('admin/products/manage/' . $id_kursus);
        }
    }
    ...
    {
        if ($this->product->is_course_exist($id)) {
            $back = $this->db->where('id', $id)->get('kursus')->result_array();
            $id_kursus = $back[0]['id_kursus'];
            $this->db->where('id', $id)->delete('kursus');
            ...
            // $this->load->view('header', $params);
            // $this->load->view('products/deletecourse', $product);
            // $this->load->view('footer');
        } else {
            show_404();
        }
    ...
    public function view($id = 0)
    {
        if ($this->product->is_product_exist($id)) {
            ...
            $product['activation'] = $this->db->where('id_kursus', $id)->get('activation')->result_array();
            $product['customers'] = $this->db->get('customers')->result_array();
